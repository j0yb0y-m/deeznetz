...
func RunCapture(args []string) {
	...
	iface := fs.String("i", "", "Network interface to capture on (default: first non-loopback interface)")
	filter := fs.String("f", "", "BPF filter expression (e.g., 'tcp port 80')")
	output := fs.String("o", "", "Output .pcap file path")
	snapLen := fs.Int("s", 1600, "Snapshot length in bytes")
	fs.Parse(args)
